...
const TAB_HEADERS = {
  ...
  Documents: ['id','projectId','projectName','customer','entity','currentStage','base','docNos','docDates','validity','rev','dueDate','payMethod','lineItems','paidAt'],
...
async function ensureTab(sheets, tabName) {
  ...
  // Tab exists — check if row 1 has the correct headers (and all expected columns)
  ...
  const firstCell = currentHeaders[0] || '';

  const needsRewrite = firstCell !== 'id' || currentHeaders.length < expectedHeaders.length;
  if (needsRewrite) {
    // If A1 looks like real data (length > 3, e.g. 'PRJ-001'), preserve it by inserting a header row above.
    if (firstCell !== 'id' && firstCell.length > 3) {
      await sheets.spreadsheets.batchUpdate({
        spreadsheetId: SHEET_ID,
        requestBody: {
          requests: [{
