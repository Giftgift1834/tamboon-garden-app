                ...
                onClick={() => {
                  ...
                  for (let pi = 0; pi < totalPages; pi++) {
                    ...
                    const cells = pg.map((p, i) => {
                      ...
                      if (!p) return `<div style="display:flex;justify-content:center"><div style="border:1px dashed #ddd;border-radius:4px;padding:2mm;width:fit-content"><div style="width:54mm;height:54mm;display:flex;align-items:center;justify-content:center;font-size:10px;color:#aaa">ช่องที่ ${n}</div><p style="font-size:10px;height:13px;width:54mm;margin-top:1mm"></p></div></div>`;
                      const pos = (docPhotoPos || {})[p.id] || { x: 50, y: 50 };
                      const zm = (docPhotoZoom || {})[p.id] || 1;
                      return `<div style="display:flex;justify-content:center"><div style="border:1px solid #ccc;border-radius:4px;padding:2mm;width:fit-content"><div style="width:54mm;height:54mm;position:relative;overflow:hidden;border-radius:2px"><img src="${p.url}" style="width:100%;height:100%;object-fit:cover;object-position:${pos.x}% ${pos.y}%;transform:scale(${zm});transform-origin:${pos.x}% ${pos.y}%;display:block"/><span style="position:absolute;top:4px;left:4px;font-size:9px;padding:1px 5px;border-radius:3px;background:rgba(0,0,0,0.55);color:#fff;font-weight:700">${n}</span></div><p style="font-size:10px;color:#444;height:13px;width:54mm;margin-top:1mm;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.caption || ''}</p></div></div>`;
                    }).join('');
