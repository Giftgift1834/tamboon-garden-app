  ...
  return (
    <div className="tg-modal-backdrop">
      <div className="tg-panel tg-print-area tg-scroll" style={{ maxWidth: 760, width: '100%', margin: '0.5rem 0', maxHeight: '94vh', overflowY: 'auto', padding: '36px 40px', background: '#fff' }}>
        ...
        {/* Signatures */}
        {stage === 'receipt' ? (
          <div className="grid grid-cols-3 gap-6 mt-10 text-center text-xs tg-sig-area" style={{ color: 'var(--moss)' }}>
            <div>
          ...
          <div className="grid grid-cols-2 gap-10 mt-10 text-center text-xs tg-sig-area" style={{ color: 'var(--moss)' }}>
            <div>
              <div className="flex justify-center mb-1" style={{ height: 32 }}>
                {stage === 'quotation' && <SignatureMark style={{ height: '100%', width: 'auto' }} />}
              </div>
                ...
                onClick={() => {
                  ...
                  for (let pi = 0; pi < totalPages; pi++) {
                    ...
                    const cells = pg.map((p, i) => {
                      ...
                      if (!p) return `<div style="border:1px dashed #ddd;border-radius:4px;overflow:hidden;display:flex;flex-direction:column;align-items:center;padding:4px 4px 2px"><div style="width:60mm;height:60mm;display:flex;align-items:center;justify-content:center;font-size:10px;color:#aaa;border:1px dashed #ccc">ช่องที่ ${n}</div><p style="font-size:10px;padding:2px 4px;height:14px;width:100%"></p></div>`;
                      const pos = (docPhotoPos || {})[p.id] || { x: 50, y: 50 };
                      const zm = (docPhotoZoom || {})[p.id] || 1;
                      return `<div style="border:1px solid #ccc;border-radius:4px;overflow:hidden;display:flex;flex-direction:column;align-items:center;padding:4px 4px 2px"><div style="width:60mm;height:60mm;position:relative;overflow:hidden;flex-shrink:0"><img src="${p.url}" style="width:100%;height:100%;object-fit:cover;object-position:${pos.x}% ${pos.y}%;transform:scale(${zm});transform-origin:${pos.x}% ${pos.y}%;display:block"/><span style="position:absolute;top:4px;left:4px;font-size:9px;padding:1px 5px;border-radius:3px;background:rgba(0,0,0,0.55);color:#fff;font-weight:700">${n}</span></div><p style="font-size:10px;padding:2px 4px;color:#444;height:14px;width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.caption || ''}</p></div>`;
                    }).join('');
              ...
              <button
                onClick={() => {
                  ...
                  for (let pi = 0; pi < totalPages; pi++) {
                    ...
                    const cells = pg.map((p, i) => {
                      const n = pi * PER_PAGE + i + 1;
                      if (!p) return `<div style="border:1px dashed #ddd;border-radius:4px;overflow:hidden"><div style="height:40mm;display:flex;align-items:center;justify-content:center;font-size:10px;color:#aaa">ช่องที่ ${n}</div><p style="font-size:11px;padding:3px 6px;min-height:1.3rem"></p></div>`;
                      const pos = (photoPos[p.id] || { x: 50, y: 50 });
                      const zm = photoZoom[p.id] || 1;
                      return `<div style="border:1px solid #ccc;border-radius:4px;overflow:hidden"><div style="height:40mm;position:relative;overflow:hidden"><img src="${p.url}" style="width:100%;height:100%;object-fit:cover;object-position:${pos.x}% ${pos.y}%;transform:scale(${zm});transform-origin:${pos.x}% ${pos.y}%;display:block"/><span style="position:absolute;top:4px;left:4px;font-size:9px;padding:1px 5px;border-radius:3px;background:rgba(0,0,0,0.55);color:#fff;font-weight:700">${n}</span></div><p style="font-size:11px;padding:3px 6px;color:#444;min-height:1.3rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.caption || ''}</p></div>`;
                    }).join('');
                    pagesHtml += `<div style="${pi > 0 ? 'break-before:page;page-break-before:always;' : ''}">
