                ...
                onClick={() => {
                  ...
                  for (let pi = 0; pi < totalPages; pi++) {
                    ...
                    const cells = pg.map((p, i) => {
                      ...
                      if (!p) return `<div style="border:1px dashed #ddd;border-radius:4px;overflow:hidden"><div style="height:52mm;display:flex;align-items:center;justify-content:center;font-size:10px;color:#aaa">ช่องที่ ${n}</div><p style="font-size:11px;padding:3px 6px;min-height:18px"></p></div>`;
                      const pos = (docPhotoPos || {})[p.id] || { x: 50, y: 50 };
                      const zm = (docPhotoZoom || {})[p.id] || 1;
                      return `<div style="border:1px solid #ccc;border-radius:4px;overflow:hidden"><div style="height:52mm;position:relative;overflow:hidden"><img src="${p.url}" style="width:100%;height:100%;object-fit:cover;object-position:${pos.x}% ${pos.y}%;transform:scale(${zm});transform-origin:${pos.x}% ${pos.y}%;display:block"/><span style="position:absolute;top:4px;left:4px;font-size:9px;padding:1px 5px;border-radius:3px;background:rgba(0,0,0,0.55);color:#fff;font-weight:700">${n}</span></div><p style="font-size:11px;padding:3px 6px;color:#444;height:18px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.caption || ''}</p></div>`;
                    }).join('');
